...
public class GitHubController {
    ...
    public ResponseEntity<?> getContents(
        ...
        String targetRef = (ref != null && !ref.isEmpty()) ? ref : "master"; // Default to master or HEAD

        System.out.println("Requested content for path: " + subPath + " ref: " + targetRef);

        try {
            // Try as directory first
            try {
                List<FileEntry> entries = gitService.listFiles(subPath, targetRef);
                // It is a directory
                List<GitHubContent> response = new ArrayList<>();
                for (FileEntry entry : entries) {
                    if (entry.getName().equals(".git")) continue;
                    response.add(mapToGitHubContent(owner, repo, entry, targetRef));
                }
                return ResponseEntity.ok(response);
            } catch (IOException e) {
                // Not a directory or path not found, try as file
            }

            // If not directory, try file
            byte[] contentBytes = gitService.getFileContent(subPath, targetRef);
            GitHubContent content = mapToGitHubContent(owner, repo, subPath, contentBytes, targetRef);
    ...
    private GitHubContent mapToGitHubContent(String owner, String repo, FileEntry entry, String ref) {
        ...
        content.setDownload_url("http://localhost:8080/repos/" + owner + "/" + repo + "/contents/" + entry.getPath() + "?ref=" + ref);
    ...
    private GitHubContent mapToGitHubContent(String owner, String repo, String path, byte[] bytes, String ref) {
        ...
        content.setGit_url("http://localhost:8080/repos/" + owner + "/" + repo + "/git/blobs/mock-sha");
        content.setDownload_url("http://localhost:8080/repos/" + owner + "/" + repo + "/contents/" + path + "?ref=" + ref);
